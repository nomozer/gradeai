import type { I18nStrings } from "../types";

export const vi: I18nStrings = {
  title: "Mirror",
  subtitle: "Chấm Bài Luận AI",
  langSwitch: "English",
  stepUpload: "Tải lên",
  stepReading: "Đọc",
  stepReview: "Chấm điểm",
  stepRegrade: "Chấm lại",
  stepDone: "Xong",
  step1Title: "Tải Bài Lên",
  step1Desc: "Nhập đề bài và ảnh bài làm của học sinh",
  promptLabel: "Đề Bài",
  promptDrop: "Thả file PDF đề bài vào đây, hoặc nhấp để chọn",
  promptChange: "Đổi PDF",
  promptUploaded: "Đã tải PDF đề bài",
  imageLabel: "Bài Làm Học Sinh",
  imageDrop: "Thả ảnh hoặc PDF vào đây, hoặc nhấp để chọn",
  imageChange: "Đổi file",
  pdfUploaded: "Đã tải PDF",
  uploadInvalidType: "Định dạng không hỗ trợ. Vui lòng chọn JPG, PNG hoặc PDF.",
  uploadReadError: "Không thể đọc tệp đã chọn. Vui lòng thử lại.",
  startGrading: "Bắt Đầu Chấm",
  queueTitle: "Danh sách bài làm",
  queueSearch: "Tìm bài đang mở…",
  queueEmpty: "Không có bài phù hợp với bộ lọc này.",
  deskTitle: "Bàn Chấm Mirror",
  deskDescription:
    "Mở bài làm, quan sát giấy thi và điều hướng vòng chấm AI ngay trên một bàn làm việc.",
  deskBadge: "Human-in-the-loop VLM",
  paperTitle: "Bài Làm Học Sinh",
  paperEmpty: "Tải ảnh quét hoặc PDF để bắt đầu bàn chấm.",
  promptCardTitle: "Phiếu Đề Bài",
  uploadCardTitle: "Tiếp Nhận Bài Nộp",
  rubricPanelTitle: "Bảng Rubric",
  rubricPanelHint: "Tải và chấm bài để kích hoạt bảng tiêu chí chi tiết.",
  statusTitle: "Trạng thái quy trình",
  noteTitle: "Ghi chú AI",
  noFile: "Chưa có tệp",
  sourceTask: "Đề bài",
  sourceEssay: "Bài làm",
  sourceTranscript: "Bản AI đọc",
  sourceRounds: "Lượt chấm lại",
  stageReady: "Sẵn sàng tiếp nhận",
  stageReading: "AI đang đọc",
  stageReview: "Chờ giáo viên xem",
  stageDone: "Đã duyệt",
  startHint: "AI sẽ đọc cả bài làm lẫn rubric trực tiếp từ bàn chấm này.",
  teacherPanelTitle: "Điều Khiển Giáo Viên",
  approvedTitle: "Kết Quả Cuối",
  approvedHint: "Bài này đã được duyệt và sẵn sàng lưu trữ hoặc xuất.",
  step2Title: "AI Đang Đọc",
  step2Desc: "Mô hình AI đang phân tích bài luận. Xin chờ…",
  step3Title: "Xem Xét & Chốt Điểm",
  step3Desc: "Xem điểm AI chấm, điều chỉnh trực tiếp và hoàn tất bài làm",
  overallScore: "Tổng Điểm",
  outOf: "/ 10",
  rubric: "Điểm Tiêu Chí",
  rubricContent: "Nội dung",
  rubricArgument: "Lập luận",
  rubricExpression: "Diễn đạt",
  rubricCreativity: "Sáng tạo",
  rubricBySubject: {
    literature: {
      content: "Nội dung",
      argument: "Lập luận",
      expression: "Diễn đạt",
      creativity: "Sáng tạo",
    },
    stem: {
      content: "Tính chính xác",
      argument: "Phương pháp",
      expression: "Trình bày",
      creativity: "Hiểu bản chất",
    },
    language: {
      content: "Hoàn thành",
      argument: "Ngữ pháp",
      expression: "Từ vựng",
      creativity: "Lưu loát",
    },
    history: {
      content: "Sự kiện",
      argument: "Phân tích",
      expression: "Trình bày",
      creativity: "Bối cảnh",
    },
  },
  subjectLabel: "Môn",
  subjectNames: {
    literature: "Ngữ văn",
    stem: "Toán / Tin / Lý",
    language: "Ngoại ngữ",
    history: "Lịch sử / GDCD",
  },
  strengths: "Ưu Điểm",
  weaknesses: "Hạn Chế",
  comment: "Nhận Xét Giám Khảo",
  transcript: "Bản AI đọc được",
  approve: "Duyệt",
  revise: "Yêu Cầu Sửa",
  reject: "Từ Chối",
  feedbackPlaceholder: "Giải thích cần thay đổi gì…",
  approveAndFinish: "Duyệt & Hoàn Thành",
  regrade: "Chấm Lại",
  regradeHint: "Phản hồi sẽ được lưu và AI sẽ chấm lại với sửa đổi của bạn",
  feedbackSaving: "Đang lưu…",
  finalizeSaving: "Đang lưu điểm…",
  finalizeSaveError: "Không thể lưu điểm cuối cùng. Vui lòng thử lại.",
  needComment: "Vui lòng giải thích khi Sửa hoặc Từ chối.",
  step4Title: "Đang Chấm Lại",
  step4Desc: "AI đang đọc lại với sửa đổi của bạn…",
  step5Title: "Hoàn Thành",
  step5Desc: "Tất cả bài đã được chấm",
  newEssay: "+ Bài Mới",
  essayN: "Bài",
  noResult: "Chưa có kết quả",
  progress: "Tiến trình",
  pipelineError: "Có lỗi xảy ra",
  viewResult: "Xem Kết Quả",
  backToReview: "Quay Lại Xem",
  grading: "Đang chấm…",
  done: "Xong",
  idle: "Sẵn sàng",
  reset: "Xóa Tất Cả",
  studentAnswer: "Câu trả lời học sinh",
  studentNamePlaceholder: "Chưa rõ tên học sinh",
  questionScore: "Điểm",
  teacherNote: "Nhận xét giáo viên",
  teacherNotePlaceholder: "Nhập nhận xét cho câu này…",
  sendComment: "Gửi",
  aiAnalyzing: "AI đang phân tích...",
  noIssues: "Không có vấn đề cần báo cáo.",
  noCommentSalvaged: "Phản hồi cho câu này bị cắt — hãy đối chiếu bài làm hoặc chấm lại.",
  salvagedTitle: "Kết quả chấm chưa đầy đủ",
  salvagedBody:
    "Mô hình đã trả về JSON không hợp lệ — nội dung bên dưới được trích xuất từng phần. Hãy kiểm tra kỹ trước khi duyệt, hoặc chấm lại bài.",
  salvagedFinalizeTitle: "Điểm AI không đáng tin",
  salvagedFinalizeBody:
    "AI không hoàn tất chấm — điểm tự động đặt về 0. Hãy tự nhập điểm dựa trên bài làm, hoặc quay lại chấm lại trước khi xác nhận.",
  verdictDisputeTitle: "AI không đồng tình",
  verdictDisputeHint:
    "AI cho rằng nhận xét này có thể không khớp bài làm thực tế. Đọc kỹ phân tích trên rồi chọn:",
  verdictDisputeApply: "Áp dụng góp ý",
  verdictDisputeSkip: "Bỏ qua, không lưu bài học",
  verdictDisputeSkipShort: "Bỏ qua",
  verdictDisputeApplied: "Đã chọn áp dụng — bài học sẽ lưu khi duyệt.",
  verdictDisputeSkipped: "Đã bỏ qua — bài học KHÔNG lưu vào bộ nhớ.",
  appliedLabel: "đã áp dụng",
  skippedLabel: "đã bỏ qua",
  teacherCommentLabel: "Nhận xét của giáo viên",
  verdictPartialBadge: "AI đồng tình một phần",
  aiVoiceLabel: "AI đã đọc",
  teacherVoiceLabel: "Giáo viên",
  dropHereTask: "Thả file đề bài vào đây!",
  dropHereEssay: "Thả file bài làm vào đây!",
  documentTabs: "Các bài làm đang mở",
  rename: "Đổi tên",
  renameTitle: "Đổi tên bài làm",
  renamePrompt: "Nhập tên mới cho bài làm này",
  cancel: "Hủy",
  save: "Lưu",
  closeTab: "Đóng thẻ",
  commentLabelGood: "Ưu điểm",
  commentLabelError: "Cần khắc phục",
  commentLabelNote: "Ghi chú",
  commentLabelWarn: "Cảnh báo",
  answerKeyLabel: "Đáp Án & Hướng Dẫn Chấm",
  answerKeyDrop: "Thả file PDF đáp án vào đây, hoặc nhấp để chọn",
  answerKeyUploaded: "Đã tải đáp án",
  answerKeyChange: "Đổi đáp án",
  dropHereAnswerKey: "Thả file đáp án vào đây!",
  answerKeyDesc: "Tải file PDF đáp án chính thức (bareme) để AI chấm điểm chính xác và sát với yêu cầu của giáo viên.",
  optionalLabel: "Tùy chọn",
  uploadFileBtn: "Tải PDF lên",
  answerKeyUploadedTitle: "Đã tích hợp Đáp án",
  activeLabel: "Đang dùng",
  backToUpload: "Quay lại",
  editGrade: "Sửa lại",
  // OriginalImageModal (Xem PDF gốc / bài làm gốc)
  close: "Đóng",
  originalImage: "Bài làm gốc",
  originalImageOpening: "Đang mở bài làm gốc…",
  originalImageInlineError: "Tệp PDF không hiển thị? Mở ở tab mới.",
  originalImageOpenNewTab: "Mở ở tab mới",
  mobileHintTitle: "Trải nghiệm màn hình lớn",
  mobileHintText:
    "Giao diện tối ưu cho màn hình lớn (máy tính/laptop). Bạn vẫn dùng được trên thiết bị này, nhưng chấm bài sẽ thoải mái hơn trên màn lớn.",
  mobileHintDismiss: "Đã hiểu",
};
