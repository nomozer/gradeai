...
const S = {
  unclear: { ...pillBase, background: "#fee2e2", color: "#b91c1c" },
  strike: {
    ...pillBase,
    background: "#fed7aa",
    color: "#9a3412",
...
function renderToken(tok, isStem, key) {
  if (tok === "[?]") {
    return (
  ...
  if (tok.startsWith("[gạch")) {
    if (tok === "[gạch]") {
      return (
        <span key={key} style={{ ...S.strike, textDecoration: "none" }} title="Nét gạch xóa không đọc được">
          [xóa bỏ]
        </span>
      );
    }
...
function tokenize(src, isStem, keyBase) {
  const nodes = [];
  let last = 0;
  let m;
  let k = 0;
...
/**
 * @param {string} text
 * @param {"literature"|"stem"|"language"|"history"} subject
 * @returns {React.ReactNode}
 */
export function formatTranscript(text, subject) {
  if (!text) return text;
