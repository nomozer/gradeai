  ...
  const handleTaskFile = useCallback(
    async (file) => {
      ...
      try {
        ...
        setUploadError(null);
        setTaskPdf({ dataUrl, name: file.name });
      } catch {
  ...
  const handleEssayFile = useCallback(
    async (file) => {
      ...
      try {
        const dataUrl = await readFileAsDataUrl(file);
        setUploadError(null);
        setEssayImage({ dataUrl, name: file.name, isPdf: check.isPdf });
      } catch {
        setUploadError(t.uploadReadError);
      }
    },
  ...
  return (
    ...
    >
      ...
      <div style={{ marginBottom: 24 }}>
        ...
        <div
          onDragOver={(e) => { e.preventDefault(); setDragOverTask(true); }}
          ...
          onClick={() => { if (taskInputRef.current) { taskInputRef.current.value = ""; taskInputRef.current.click(); } }}
          style={{
        ...
        >
          <input
            ...
            style={{ display: "none" }}
            onChange={(e) => { handleTaskFile(e.target.files?.[0]); e.target.value = ""; }}
          />
          {taskPdf ? (
            <div>
              <div
              ...
              >
                ...
                >
                  ...
                  <div style={{ fontSize: 11, fontWeight: 800, marginTop: 6, letterSpacing: "0.1em" }}>PDF</div>
                </div>
                <div style={{ fontSize: 15, color: T.textSoft, fontWeight: 500 }}>{t.promptUploaded}</div>
              </div>
              <div style={{ marginTop: 6, fontSize: 14, color: T.textMute, textAlign: "center" }}>
                {taskPdf.name} · <span style={{ color: T.accent, cursor: "pointer" }}>{t.promptChange}</span>
              </div>
            </div>
          ) : (
            <div>
              ...
              <div style={{ fontSize: 17, color: T.textSoft, fontWeight: 500 }}>{t.promptDrop}</div>
      ...
      <div style={{ marginBottom: 32 }}>
        ...
        <div
          onDragOver={(e) => { e.preventDefault(); setDragOverEssay(true); }}
          ...
          onClick={() => { if (essayInputRef.current) { essayInputRef.current.value = ""; essayInputRef.current.click(); } }}
          style={{
        ...
        >
          <input
            ...
            style={{ display: "none" }}
            onChange={(e) => { handleEssayFile(e.target.files?.[0]); e.target.value = ""; }}
          />
          {essayImage ? (
            <div>
              {essayImage.isPdf ? (
                ...
                >
                  ...
                  >
                    ...
                    <div style={{ fontSize: 13, fontWeight: 800, marginTop: 8, letterSpacing: "0.1em" }}>PDF</div>
                  </div>
                  <div style={{ fontSize: 15, color: T.textSoft, fontWeight: 500 }}>{t.pdfUploaded}</div>
                </div>
              ) : (
                <img
              ...
              <div style={{ marginTop: 10, fontSize: 14, color: T.textMute, textAlign: "center" }}>
                {essayImage.name} · <span style={{ color: T.accent, cursor: "pointer" }}>{t.imageChange}</span>
              </div>
            </div>
          ) : (
            <div>
              ...
              <div style={{ fontSize: 17, color: T.textSoft, fontWeight: 500 }}>{t.imageDrop}</div>
